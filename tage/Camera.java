...
public class Camera
{
	...
	/** position Camera with Obj */
	public void positionCameraWObj(GameObject obj, float addUp, float addFwd, float addRight) {
		Vector3f loc, fwd, up, right;

		loc = obj.getWorldLocation();
		fwd = obj.getWorldForwardVector();
		up = obj.getWorldUpVector();
		right = obj.getWorldRightVector();
		setU(right);
		setV(up);
		setN(fwd);
		setLocation(loc.add(up.mul(addUp)).add(fwd.mul(addFwd)).add(right.mul(addRight)));
	}
	...
	public void turnRightAction() {
		...
		upVector = getV();
		fwdVector = getN();
		rightVector.rotateAxis(-0.01f, upVector.x(), upVector.y(), upVector.z());
		fwdVector.rotateAxis(-0.01f, upVector.x(), upVector.y(), upVector.z());
		setU(rightVector);
		setN(fwdVector);
	}
